...
import { purchaseElectricity } from "@/lib/alrahuz";

const purchaseSchema = z.object({
  buyerPhone: z.string().regex(/^0[0-9]{10}$/, "Invalid buyer phone"),
  providerId: z.string().min(1, "Select electricity provider"),
  meterNumber: z.string().min(5, "Enter meter number").max(30, "Meter number is too long"),
  meterType: z.enum(["prepaid", "postpaid"]),
  amount: z.number().int().min(100, "Enter a valid amount"),
...
export async function POST(req: NextRequest) {
  try {
    ...
    const rateLimitError = enforceRateLimit(req, "airtimePurchase", "/api/electricity/purchase");
    if (rateLimitError) return rateLimitError;

    const { buyerPhone, providerId, meterNumber, meterType, amount, pin, confirmDuplicate = false } =
    ...
    const provider = await prisma.electricityProvider.findUnique({ where: { id: providerId } });
    if (!provider || !provider.isActive) {
      return NextResponse.json({ success: false, error: "Electricity provider is unavailable" }, { status: 404 });
    }

    if (amount < provider.minAmount || amount > provider.maxAmount) {
      return NextResponse.json(
        { success: false, error: `Amount must be between N${provider.minAmount.toLocaleString()} and N${provider.maxAmount.toLocaleString()}` },
        { status: 400 }
      );
    }
    ...
    const reference = `ELECTRICITY-${user.id}-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`;
    const lockKey = buildPurchaseLockKey("electricity", user.id, `${providerId}:${meterNumber}`, amount);
    ...
    const txResult = await prisma.$transaction(async (tx) => {
      ...
      await tx.transaction.create({
        data: {
          ...
          status: "PENDING",
          reference,
          description: `${provider.name} ${meterType} meter: ${meterNumber}`,
          phone: meterNumber,
          apiUsed: "API_C",
    ...
    if (txResult.kind === "pending_duplicate") {
      return NextResponse.json({ success: false, error: "A similar electricity purchase is already processing.", duplicateTransaction: txResult.duplicateTransaction }, { status: 409 });
    }
    ...
    const apiResult = await purchaseElectricity({
      discoName: provider.discoName,
      amount,
      meterNumber,
