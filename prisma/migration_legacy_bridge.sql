-- ============================================================================
-- MK DATA - Legacy Neon Bridge Migration
-- Purpose:
  ...
  IF to_regclass('"UserReservedAccount"') IS NOT NULL THEN
    ...
    normalized AS (
      SELECT
        ...
        COALESCE(NULLIF(r."bankName", ''), NULLIF(u.bank_name, ''), 'MK Data WALLET') AS "bankName",
        COALESCE(NULLIF(r."providerReference", ''), CONCAT('MKDATA-VA-', r."userId", '-', EXTRACT(EPOCH FROM COALESCE(r."createdAt", NOW()))::BIGINT)) AS "orderRef",
...
BEGIN
  IF to_regclass('"Transaction"') IS NOT NULL THEN
    ...
    SELECT
      ...
      CASE WHEN d.tx_type = 'WALLET_FUNDING'::"TransactionType" AND d.ref LIKE 'MKDATA-VA-%' THEN d.ref ELSE NULL END,
      NULL,
      CASE
